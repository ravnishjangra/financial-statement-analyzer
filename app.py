...
class FactorInvesting:
    ...
    def analyze_factor_exposure(analyzer):
        exposures = {}
        info = analyzer.financials.get('info', {}); ratios = analyzer.ratios; prices = analyzer.financials.get('prices')
        ...
        if mcap and mcap>0:
            amt = analyzer._format_amount(mcap)
            if mcap>1e11: exposures['Size'] = {'score':'Mega Cap','detail':amt,'color':'#94a3b8'}
            elif mcap>1e10: exposures['Size'] = {'score':'Large Cap','detail':amt,'color':'#667eea'}
            elif mcap>2e9: exposures['Size'] = {'score':'Mid Cap','detail':amt,'color':'#f59e0b'}
            else: exposures['Size'] = {'score':'Small Cap','detail':amt,'color':'#10b981'}
        if prices is not None and not prices.empty and 'Close' in prices.columns and len(prices)>=252:
            ret12 = (prices['Close'].iloc[-1]/prices['Close'].iloc[-252]-1)*100
            if ret12>30: exposures['Momentum'] = {'score':'Strong','detail':f'12M:{ret12:.0f}%','color':'#10b981'}
            elif ret12>10: exposures['Momentum'] = {'score':'Positive','detail':f'12M:{ret12:.0f}%','color':'#667eea'}
            elif ret12>-10: exposures['Momentum'] = {'score':'Neutral','detail':f'12M:{ret12:.0f}%','color':'#f59e0b'}
            else: exposures['Momentum'] = {'score':'Negative','detail':f'12M:{ret12:.0f}%','color':'#ef4444'}
